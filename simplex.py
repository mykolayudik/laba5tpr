...
class Simplex(object):
    ...
    def objective_maximize(self):
        ...
        print(Fore.BLUE + "Симплекс-таблиця:" + Style.RESET_ALL)
        self.print_simplex_table()

        print(Fore.BLUE + "\nПокрокове розв'язання симплекс методом:" + Style.RESET_ALL)
        while condition is True:
            key_row = self.find_key_row(key_column = key_column)
            self.basic_vars[key_row] = key_column
            pivot = self.coeff_matrix[key_row][key_column]
        ...
        print("\n" + Fore.BLUE + 'Кінцевий вигляд симплекс-таблиці:' + Style.RESET_ALL)
        self.print_simplex_table() 
        solution = {}
